...
impl HtmlParser {
    pub fn new(t: HtmlTokenizer) -> Self {
        Self {
            ...
            original_insertion_mode: InsertionMode::Initial, // 元の状態に戻る時などに使う
                    ...
                    match token {
                        ...
                        Some(HtmlToken::StartTag {
                            ...
                            if tag == "style" || tag == "script" {
                                self.insert_element(tag, attributes.to_vec());
                                // Style / Scriptタグを読み込んだ後に InHead 状態として戻るために original_insertion_mode に現在状態を入れている
                            ...
                            match tag.as_str() {
                                "body" => {
                                    ...
                                    if !self.contain_in_stack(ElementKind::Body) {
                                        // bodyの開始タグが見つからなかったためパース失敗．トークンを無視する
                                        continue;
                                    }
    ...
    fn insert_element(&mut self, tag: &str, attributes: Vec<Attribute>) {
        ...
        // stackはLIFOなのでlast()を取得することは現在注目しているノードを取得することを意味する
    ...
    fn pop_current_node(&mut self, element_kind: ElementKind) -> bool {
        ...
        if current.borrow_mut().element_kind() == Some(element_kind) {
...
/// https://html.spec.whatwg.org/multipage/parsing.html#the-insertion-modes
...
mod tests {
    ...
    #[test]
    fn test_body() {
        let html = "<html><head></head><body></body></htmp>".to_string();
        let t = HtmlTokenizer::new(html);
        let window = HtmlParser::new(t).construct_tree();
        ...
        let html = document.borrow().first_child().expect("failed to get");
        assert_eq!(
        ...
        let head = html.borrow().first_child().expect("failed to get");
        assert_eq!(
            Rc::new(RefCell::new(Node::new(NodeKind::Element(Element::new(
                "head",
                Vec::new()
        ...
        let body = head.borrow().next_sibling().expect("failed to get");
        assert_eq!(
            Rc::new(RefCell::new(Node::new(NodeKind::Element(Element::new(
                "body",
                Vec::new()
    ...
    #[test]
    fn test_text() {
        let html = "<html><head></head><body>text</body></htmp>".to_string();
        let t = HtmlTokenizer::new(html);
        let window = HtmlParser::new(t).construct_tree();
        let document = window.borrow().document();
        ...
        let text = body.borrow().first_child().expect("failed to get");
        assert_eq!(
    ...
    fn test_multiple_nodes() {
        let html = "<html><head></head><body><p><a foo=bar>text</a></p></body></htmp>".to_string();
        let t = HtmlTokenizer::new(html);
        let window = HtmlParser::new(t).construct_tree();
        let document = window.borrow().document();
        ...
        let p = body.borrow().first_child().expect("failed to get");
        assert_eq!(
            Rc::new(RefCell::new(Node::new(NodeKind::Element(Element::new(
                "p",
                Vec::new()
        ...
        let a = p.borrow().first_child().expect("failed to get");
        assert_eq!(
            Rc::new(RefCell::new(Node::new(NodeKind::Element(Element::new(
                "a",
                vec![attr],
        ...
        let text = a.borrow().first_child().expect("failed to get");
        assert_eq!(
            Rc::new(RefCell::new(Node::new(NodeKind::Text("text".to_string())))),
            text
        );
