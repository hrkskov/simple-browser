...
impl Iterator for HtmlTokenizer {
    ...
    // 各ステートに対応する処理をmatchで分岐させて実装する
    // 各ステートでの処理は基本的に次の文字を消費し，その文字に応じて状態遷移を行う
    // トークンが完成したら，最新のトークンを返す
    // トークンが完成しない場合は，ループを継続して次の文字を処理する
    // 入力が終了した場合は，EOFトークンを返す
    // reconsumeフラグが立っている場合は，次の文字を消費せずに同じ文字を再度処理する
        ...
        loop {
            ...
            match self.state {
                ...
                State::BeforeAttributeName => {
                    if c == '/' || c == '>' || self.is_eof() {
                        // ここで同じトークンに対してreconsumeすることで，今まで出現したトークンを利用してAttributeを作成する
                        self.reconsume = true;
                ...
                State::AfterAttributeName => {
                    ...
                    if c == '>' {
                        // タグの終了が正常終了した場合
                        // Dataステートに戻り，最新のHtmlToken（タグトークン）を返す
                        self.state = State::Data;
                        return self.take_latest_token();
                    }
                ...
                State::TemporaryBuffer => {
                    ...
                    let c = self
                        ...
                        .chars()
                        .nth(0)
                        .expect("self.buf should not be empty");
                    self.buf.remove(0);
                    return Some(HtmlToken::Char(c));
                }
            }
...
mod test {
    ...
    fn test_self_closing_tag() {
        let html = "<img src=\"image.png\" />".to_string();
        let mut tokenizer = HtmlTokenizer::new(html);
        let mut attr = Attribute::new();
        attr.add_char('s', true);
        attr.add_char('r', true);
        attr.add_char('c', true);
        attr.add_char('i', false);
        attr.add_char('m', false);
        attr.add_char('a', false);
        attr.add_char('g', false);
        attr.add_char('e', false);
        attr.add_char('.', false);
        attr.add_char('p', false);
        attr.add_char('n', false);
        attr.add_char('g', false);
